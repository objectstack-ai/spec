...
> **Goal:** Implement the remaining 19 service contracts as production-ready plugins.

### Priority 1 — Essential Services

| Contract | Priority | Notes |
|:---|:---:|:---|
| `ICacheService` | **P0** | Redis / Memory cache — needed for session, metadata, query caching |
| `IQueueService` | **P0** | BullMQ / Redis Streams — needed for async workflows and jobs |
| `IJobService` | **P0** | Cron/interval scheduling — depends on IQueueService |
| `IStorageService` | **P1** | S3 / Azure Blob / Local FS — file upload/download for apps |
| `ISchemaDriver` | **P1** | DDL operations — needed for `objectstack migrate` CLI command |

- [ ] `plugin-cache` — Implement `ICacheService` with Redis + in-memory fallback
- [ ] `plugin-queue` — Implement `IQueueService` with BullMQ adapter
- [ ] `plugin-job` — Implement `IJobService` with cron scheduling
- [ ] `plugin-storage` — Implement `IStorageService` with S3/local adapters
...
| Contract | Priority | Notes |
|:---|:---:|:---|
| `IRealtimeService` | **P1** | WebSocket/SSE pub/sub for live data |
| `INotificationService` | **P2** | Email/SMS/Push/Slack/Teams/Webhook |
| `IGraphQLService` | **P2** | Auto-generated GraphQL from object schemas |
| `II18nService` | **P2** | Runtime i18n with locale loading and translation |

- [ ] `plugin-realtime` — Implement `IRealtimeService` with WebSocket + Redis pub/sub
- [ ] `plugin-notification` — Implement `INotificationService` with channel adapters
- [ ] `plugin-graphql` — Implement `IGraphQLService` with auto-schema generation from objects
- [ ] `plugin-i18n` — Implement `II18nService` with file-based locale loading
...
| Contract | Priority | Notes |
|:---|:---:|:---|
| `IAIService` | **P2** | LLM integration (OpenAI/Anthropic/local) |
...
- [ ] `plugin-ai` — Implement `IAIService` with multi-provider LLM routing
...
- [ ] `plugin-ai` — Multi-provider LLM service (OpenAI, Anthropic, Gemini, local models)
...
| 4 | Auth Service | `IAuthService` | ✅ | `@objectstack/plugin-auth` | better-auth integration |
| 5 | HTTP Server | `IHttpServer` | ✅ | `@objectstack/plugin-hono-server` | Hono-based server |
...
| 10 | Cache Service | `ICacheService` | ❌ | — | Spec only |
| 11 | Queue Service | `IQueueService` | ❌ | — | Spec only |
| 12 | Job Service | `IJobService` | ❌ | — | Spec only |
| 13 | Storage Service | `IStorageService` | ❌ | — | Spec only |
| 14 | Realtime Service | `IRealtimeService` | ❌ | — | Spec only |
| 15 | Search Service | `ISearchService` | ❌ | — | Spec only |
| 16 | Notification Service | `INotificationService` | ❌ | — | Spec only |
| 17 | AI Service | `IAIService` | ❌ | — | Spec only |
| 18 | Automation Service | `IAutomationService` | ❌ | — | Spec only |
