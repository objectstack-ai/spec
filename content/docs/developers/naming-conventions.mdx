...
- 🗄️ **Database problems** - Collation and case-sensitivity inconsistencies
...
// Invalid
'user.created'        // ❌ Dots not allowed
'UserProfile'         // ❌ Must be lowercase
```
...
// Object Definition
{
  name: 'crm_account',           // ✅ snake_case machine identifier
  label: 'Customer Account',     // ✅ Label for display
  fields: [
    {
      name: 'annual_revenue',    // ✅ snake_case field name
      label: 'Annual Revenue',   // ✅ Label
      type: 'currency',
      maxLength: 15              // ✅ camelCase schema property
    }
  ]
}
...
// Role Definition
{
  name: 'sales_manager',         // ✅ snake_case role name
...
// Event
{
  name: 'user.created',          // ✅ dot notation for events
  payload: { userId: 123 }
}

// App Navigation
{
  name: 'app_crm',               // ✅ snake_case app ID
  label: 'CRM Application',      // ✅ Label
  navigation: [
    {
      id: 'menu_sales',          // ✅ snake_case menu ID
      label: 'Sales'             // ✅ Label
    }
  ]
}
...
// Select Option
{
  value: 'In Progress',          // ❌ Should be 'in_progress'
  label: 'In Progress'
}
```
...
// Valid
SnakeCaseIdentifierSchema.parse('crm_account');  // ✅
EventNameSchema.parse('user.created');           // ✅

// Invalid - throws ZodError
SnakeCaseIdentifierSchema.parse('CrmAccount');   // ❌ Uppercase
SnakeCaseIdentifierSchema.parse('crm-account');  // ❌ Kebab-case
EventNameSchema.parse('UserCreated');            // ❌ No dots
```
...
- [Identifier Schemas Source Code](/docs/references/shared/identifiers/SystemIdentifier) - Implementation details
