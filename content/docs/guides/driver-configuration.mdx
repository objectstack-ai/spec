...
| **MongoDB** | `@objectstack/driver-mongodb` | Document-oriented, flexible schema |
...
When `OS_DATABASE_DRIVER` is set, the CLI auto-registers `@objectstack/driver-mongodb`
in place of the default in-memory driver. Recognised aliases: `mongodb`, `mongo`.
