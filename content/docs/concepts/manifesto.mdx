...
**ObjectQL is neutral**: It must not contain any logic specific to a particular language (e.g., Node.js), database (e.g., MySQL), or runtime (e.g., Electron).

### Standards First

**Spec before Engine**: Any feature must first be defined in the Specification layer before being implemented in the Engine layer. We reject the "implement first, standardize later" black-box approach.

### What This Means

ObjectStack's protocol layer (ObjectQL, ObjectUI) consists purely of specification definitions, with no concrete implementation. This ensures:
...
- No implementation-specific logic should leak into the protocol layer
...
**The protocol remains neutral; the implementation is replaceable.**

## Principle II: Mechanism over Policy

### The Principle

**ObjectQL provides mechanisms only**: It defines the `allowRead` interface, but never mandates where user data must exist.

**ObjectOS provides implementations**: User systems, file storage, and multi-tenancy logic belong to the OS layer. Polluting the Protocol layer with these is strictly forbidden.

### Separation of Concerns

We completely separate "what to do" (mechanism) from "how to do it" (policy/implementation).
