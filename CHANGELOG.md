...
### Fixed
- **Vercel API rewrite dropped path segments** — The Vercel rewrite rule now forwards `/api/:path*` verbatim instead of collapsing every request to `/api/[[...route]]`, which was causing control-plane endpoints like `POST /api/v1/cloud/projects` to return 404s after deployment. (`apps/server/vercel.json`)
