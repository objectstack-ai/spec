...
exports.Case = spec_1.ObjectSchema.create({
    name: 'case',
    label: 'Case',
    pluralLabel: 'Cases',
    icon: 'life-buoy',
    ...
    fields: {
        // Case Information
        case_number: spec_1.Field.autonumber({
            label: 'Case Number',
            format: 'CASE-{00000}',
        }),
        ...
        contact: spec_1.Field.lookup('contact', {
            label: 'Contact',
            required: true,
            referenceFilters: ['account = {case.account}'],
        }),
        // Case Management
        status: spec_1.Field.select({
            label: 'Status',
            required: true,
            options: [
        ...
        type: spec_1.Field.select(['Question', 'Problem', 'Feature Request', 'Bug'], {
            label: 'Case Type',
        }),
        origin: spec_1.Field.select(['Email', 'Phone', 'Web', 'Chat', 'Social Media'], {
            label: 'Case Origin',
        }),
        // Assignment
        ...
        closed_date: spec_1.Field.datetime({
            label: 'Closed Date',
            readonly: true,
        }),
        first_response_date: spec_1.Field.datetime({
            label: 'First Response Date',
            readonly: true,
        }),
        resolution_time_hours: spec_1.Field.number({
            label: 'Resolution Time (Hours)',
            readonly: true,
            scale: 2,
        }),
        sla_due_date: spec_1.Field.datetime({
            label: 'SLA Due Date',
        }),
        is_sla_violated: spec_1.Field.boolean({
            label: 'SLA Violated',
            defaultValue: false,
            readonly: true,
        }),
        // Escalation
        is_escalated: spec_1.Field.boolean({
            label: 'Escalated',
            defaultValue: false,
        }),
        escalation_reason: spec_1.Field.textarea({
            label: 'Escalation Reason',
        }),
        // Related case
        parent_case: spec_1.Field.lookup('case', {
            label: 'Parent Case',
            description: 'Related parent case',
        }),
        // Resolution
        resolution: spec_1.Field.markdown({
            label: 'Resolution',
        }),
        // Customer satisfaction
        customer_rating: spec_1.Field.rating(5, {
            label: 'Customer Satisfaction',
            description: 'Customer satisfaction rating (1-5 stars)',
        }),
        customer_feedback: spec_1.Field.textarea({
            label: 'Customer Feedback',
        }),
        // Customer signature (for case resolution acknowledgment)
        customer_signature: spec_1.Field.signature({
            label: 'Customer Signature',
            description: 'Digital signature acknowledging case resolution',
        }),
        // Internal notes
        internal_notes: spec_1.Field.markdown({
            label: 'Internal Notes',
            description: 'Internal notes not visible to customer',
        }),
        // Flags
        is_closed: spec_1.Field.boolean({
            label: 'Is Closed',
            defaultValue: false,
            readonly: true,
        }),
