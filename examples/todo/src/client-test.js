...
const client_1 = require("@objectstack/client");
async function main() {
    console.log('🚀 Starting Client SDK Test...');
    // 1. Initialize Client
    const client = new client_1.ObjectStackClient({
    ...
    try {
        ...
        if (newTask && newTask.id) {
            ...
            const deleted = await client.data.delete('todo_task', newTask.id);
        ...
        advancedResult.value.forEach((task) => {
