...
import {
  ...
  BetterAuthPluginConfigSchema,
  BetterAuthConfigSchema,
  BetterAuthProviderSchema,
  type BetterAuthConfig,
  type BetterAuthProvider,
  type OAuthProvider,
} from './auth-better.zod';
...
describe('BetterAuthStrategy', () => {
  it('should accept valid authentication strategies', () => {
    ...
    strategies.forEach((strategy) => {
      expect(() => BetterAuthStrategy.parse(strategy)).not.toThrow();
    });
  });

  it('should reject invalid strategies', () => {
    expect(() => BetterAuthStrategy.parse('invalid')).toThrow();
  });
});
...
describe('SessionConfigSchema', () => {
  ...
  it('should use default values', () => {
    ...
    expect(result.cookieName).toBe('better-auth.session_token');
...
describe('CSRFConfigSchema', () => {
  ...
  it('should use default values', () => {
    ...
    expect(result.cookieName).toBe('better-auth.csrf_token');
  });
});
...
describe('BetterAuthPluginConfigSchema', () => {
  it('should accept valid plugin configuration', () => {
    ...
    expect(() => BetterAuthPluginConfigSchema.parse(config)).not.toThrow();
  });
});
...
describe('BetterAuthConfigSchema', () => {
  it('should accept minimal valid configuration', () => {
    ...
    expect(() => BetterAuthConfigSchema.parse(config)).not.toThrow();
  });
  ...
  it('should enforce snake_case for name field', () => {
    const invalidConfig = {
      name: 'betterAuth', // camelCase - invalid
      label: 'Better Auth',
      strategies: ['email_password'],
      baseUrl: 'https://example.com',
      secret: 'a'.repeat(32),
    ...
    expect(() => BetterAuthConfigSchema.parse(invalidConfig)).toThrow();

    const validConfig = {
      ...invalidConfig,
      name: 'better_auth', // snake_case - valid
    };

    expect(() => BetterAuthConfigSchema.parse(validConfig)).not.toThrow();
  });
  ...
  it('should require at least one strategy', () => {
    ...
    expect(() => BetterAuthConfigSchema.parse(config)).toThrow();
  });
  ...
  it('should use default values for optional fields', () => {
    ...
    const result = BetterAuthConfigSchema.parse(config);
...
describe('BetterAuthProviderSchema', () => {
  it('should accept valid better-auth provider', () => {
    const provider: BetterAuthProvider = {
      type: 'better_auth',
      config: {
        name: 'main_auth',
        label: 'Main Auth',
    ...
    expect(() => BetterAuthProviderSchema.parse(provider)).not.toThrow();
  });

  it('should require type to be "better_auth"', () => {
    const provider = {
    ...
    expect(() => BetterAuthProviderSchema.parse(provider)).toThrow();
  });
});

describe('Type inference', () => {
  it('should correctly infer BetterAuthConfig type', () => {
    const config: BetterAuthConfig = {
      name: 'test_auth',
      label: 'Test Auth',
      strategies: ['email_password'],
  ...
  it('should correctly infer BetterAuthProvider type', () => {
    const provider: BetterAuthProvider = {
      type: 'better_auth',
      config: {
        name: 'test_auth',
    ...
    // This test passes if TypeScript compiles without errors
    expect(provider.type).toBe('better_auth');
