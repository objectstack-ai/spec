...
/**
 * Better-Auth Authentication Protocol
 * 
 * Defines the schema for integrating better-auth (https://better-auth.com) 
 * as an authentication provider in the ObjectStack ecosystem.
 * 
 * Better-auth is a modern, framework-agnostic authentication library that provides:
 * - Multiple authentication strategies (email/password, OAuth, magic links, passkeys)
 * - Session management
...
/**
 * Supported authentication strategies in better-auth
 */
export const BetterAuthStrategy = z.enum([
...
export type BetterAuthStrategy = z.infer<typeof BetterAuthStrategy>;
...
export const SessionConfigSchema = z.object({
  ...
  cookieName: z.string().default('better-auth.session_token').describe('Session cookie name'),
...
export const CSRFConfigSchema = z.object({
  ...
  cookieName: z.string().default('better-auth.csrf_token').describe('CSRF cookie name'),
 ...
 * Maps better-auth user fields to ObjectStack user object fields
 */
 ...
 * Extends better-auth with additional features
 */
export const BetterAuthPluginConfigSchema = z.object({
...
export type BetterAuthPluginConfig = z.infer<typeof BetterAuthPluginConfigSchema>;

/**
 * Complete Better-Auth Configuration Schema
 * 
 * This is the main configuration object for integrating better-auth
 * into an ObjectStack application.
 * 
 * @example
 * ```typescript
...
export const BetterAuthConfigSchema = z.object({
   ...
   */
  strategies: z.array(BetterAuthStrategy).min(1).describe('Enabled authentication strategies'),
   ...
   */
  plugins: z.array(BetterAuthPluginConfigSchema).default([]),
...
/**
 * TypeScript type inferred from BetterAuthConfigSchema
 */
export type BetterAuthConfig = z.infer<typeof BetterAuthConfigSchema>;

/**
 * Better-Auth Provider Schema
 * Wraps the configuration for use in the identity system
 */
export const BetterAuthProviderSchema = z.object({
  type: z.literal('better_auth').describe('Provider type identifier'),
  
  config: BetterAuthConfigSchema.describe('Better-auth configuration'),
});

export type BetterAuthProvider = z.infer<typeof BetterAuthProviderSchema>;
